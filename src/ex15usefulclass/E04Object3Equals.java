...
class MyClass{
	...
	/*
	 toString() : 인스턴스가 가진 데이터를 String으로 반환
	...
	/*
	 ...
	 인스턴스가 가진 멤버변수의 값에 대한 비교를 통해 동일여부를 판단
	 equals() 메소드의 매개변수 타입은 모든 인스턴스를 대상으로 하므로 Object형으로 정의되어있다
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		 매개변수로 전달된 인스턴스를 Object로 받으면 업캐스팅이 되므로 다운캐스팅 이후에 비교를 진행해야한다
		 전달된 인스턴스가 MyClass타입이 아니라면 비교의 대상이 될 수 없으므로
		 상속관계를 확인을 위해 instanceof연산자를 사용
		 */
		if(obj instanceof MyClass) {
			/*
			 매개변수가 MyClass타입이 맞다면 비교를 위해 다운캐스팅한다
			 그렇지 않으면 부모타입으로는 자식멤버에 접근할 수 없으므로 멤버변수 data에 대한 비교가 불가능해진다
			 */
			...
			/*
			 매개변수로 전달된 인스턴스가 MyClass타입이 아니라면 비교의 대상이 될 수 없으므로 false를 반환
			 */
			System.out.println("Nyclass 객체가 아님");
			return false;
		}
	}
}
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		//해당 비교는 참조값에 대한 비교이므로 다르다는 결과가 출력
		if(mc1==mc2) {
		...
		/*
		 인스턴스의 내용을 비교하므로 같다고 출력
		 만약 오버라이딩 한 equals()를 삭제한 후 실행하면 다르다는 결과가 출력된다
		 오버라이딩이 필요 없는 경우는 기본 클래스일때만 가능하다
		 */
