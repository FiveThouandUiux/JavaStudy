...
/*
 equals() 메소드 
  - 자바에서 인스턴스의 내용 비교를 위해 비교연산자(==)를 사용하는 것은 단순히 참조값에 대한 비교를 하는 것이므로
  	실제 내용에 대한 비교가 이루어지지 않는다
  - 인스턴스간의 내용 비교를 위해서는 equals()를 각 클래스에 맞게 오버라이딩 하여 사용해야 한다
  - String과 같은 기본클래스는 별도의 오버라이딩 없이도 배용비교가 가능한다
 */
class IntNumber {
	int num;
	...
	/*
	 equals() 메소드가 어떻게 인스턴스간의 내용을 비교하는지 설명하기 위한 메소드를 정의
	 */
...
public class E04Object2MyEquals {
	...
	public static void main(String[] args) {
		...
		/*
		 String과 같은 기본클래스는 별도의 오버라이딩 없이도 내용비교과 가능하다
		 */
		String str1=new String("Java 개발자");
		String str2= "Java 개발자";
		if(str1.equals(str2)) {
			System.out.println("같은 문자열 입니다");//결과 출력
		} else {
		...
		/*
		 개발자가 직접 정의한 myEquals()메소드를 통해 인스턴스간의 내용비교를 한다
		 */
