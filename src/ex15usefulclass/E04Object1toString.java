...
/*
 Object클래스
  - 자바에서는 최상위 클래스
 - 별도의 선언없이 Object클래스에 정의된 모든 메소드르를 호출할 수 있다
  - 개발자가 정의한 모든 클래스는 Object의 참조변수로 참조할 수 있다
    Ex) Object obj = new person()
    	혹은
    	boid myFn(Object obj){}
    	
  toString() 메소드
   - Object에 정의된 메소드로 print()문이 문자열을 출력하기전에 자동으로 호출
   - 인스턴스 변수를 문자열 형태로 변환하여 반환
   - 필요한 경우 클래스 정의시 적절히 오버라이딩 하여 재정의하는 것이 좋지만 필수 사항은 아니다
 */

/*
 해당 클래스는 tosString()을 오버라이딩하여 멤버변수를 반환하도록 정의
 */
