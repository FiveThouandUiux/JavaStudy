...
public class E05Date {
	...
	/*
	 Calendar 클래스를 통해 얻은 인스턴스를 초로 변환한 후 매개변수로 전달된 날짜서식으로 변경한 String을 반환
	 */
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		 getTime() : 서버의 표준시인 1970-01-01부터 현재까지의 시간을 초 단위로 반환
		 	이를 '타임 스탬프'라고 한다
		 */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		/*
		 Calendar 클래스는 대표적인 싱글톤 클래스이므로 new로 인스턴스를 생성할 수 없다
		 유틸리티 메소드를 통해 참조값을 얻어와야 한다
		 */
		Calendar calendar = Calendar.getInstance();
		
		/*
		 월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이 동일하므로 구분을 위해 월은 대문자, 분은 소문자를 사용
		 */
		...
		/*
		 현재 날짜를 기준으로 전/후를 구하기 위해 add()메소드를 사용
		...
		//앞에서 add()를 3번 호출했으므로 결과는 아래와 같이 출력
		String nowStr3 = myDateFormat(calendar, "yyyy년 MM월 dd일");
		System.out.println("1년2개월3일후 : " + nowStr3);
		
		/*
		 올해 크리스마스까지 남은 날짜를 계싼
		 여기서는 두 가지 서식의 날짜를 차싱하여 구한 날짜 인스턴스를 통해 계산
		 */
		//오늘날짜 : 0000-00-00 형식 
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			 ...
			 따라서 각 날자의 서식을 명확히 지정해야지만 날짜를 인식하여 연산할 수 있다
			...
			/*
			 getTime()으로 얻어오는 시간은 13자인 정수값인데 10자리까지는 순수한 초(sec)이고
			 11-13자리는 밀리세컨즈를 의미한다
			 100으로 나누어 뒤의 3자리를 제거하면 순수한 초가 된다
			 하루는 86400초 이므로 아래와 같이 남은 날짜를 계산할 수 있다
			 */
