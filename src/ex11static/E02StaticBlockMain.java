...
/*
 static 블럭 
  : 동일 클래스내 main() 메소드보다 먼저 실행되는 블럭으로 main 메소드의 실행코드가 없어도 먼저 실행된다.
  생성자보다도 먼저 실행된다
   프로그램에서 전처리를 해야하는 경우 사용 할 수 있다
 */
public class E02StaticBlockMain {
	...
	static {
		...
		//블럭 내에서만 사용할 수 있는 변수로, 블럭내에서는 일반변수를 생성한 후 사용할 수 있다
		int localVar;
		localVar = 1000;
...
//		instanceMethod();
	...
	/*
	 생성자 메소드 : 생성자에서는 정적 멤버에 접근할 수 있다
	  	일반적인 멤버변수와 동일
	 */
	...
	public static void main(String[] args) {
		...
		/*
		 static 블럭내에서 선언된 변수는 지역변수이므로 main 메소드에서는 사용할 수 없다
		 해당 지역에서만 사용할 수 있다
		 */
//		System.out.println("localVar="+ localVar);
		 
		//new를 통해 해당 클래스의 인스턴스 생성한다
