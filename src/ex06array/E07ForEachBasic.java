...
/*
  foreach문(개선된 for문)
  : 배열의 일부가 아니라 전체를 참조해야할때 주로 사용
  배열의 요소를 참조의 형태로 가져와서 사용
  참조의 목적으로만 사용해야하고 값의 변경은 허용 불가
  형식] 
  	for(자료형 참조변수 : 배열명) {
  		실행문장;
  		참조변수를 통해 사용가능;
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/*
		 배열 arr의 요소를 순서대로 하나씩 참조하여 출력하고 더 이상 참조할 요소가 없을 때 자동으로 종료
		 */
		sum=0;
		for(int e : arr) {
			System.out.print(e+" ");
		...
		/*
		 참조형태로 각 원소를 읽어와서 변수 b에  할당하는 형식이므로
		  아래 증가연산자를 통해 증가 시킨 값은 실제 배열에 적용되지 않는다
		 for문 내에서만 증가된 값이 출력
		 */
		for(int b : arr) {
			//배열에서 꺼낸 후 b에 저장된 값이 증가
			b++;
			System.out.print(b +" "); // 1증가된 값이 출력
		}
		System.out.println();
		System.out.println("배열arr의 요소 재출력하기");
		// 1증가는 기본 배열에 적용되지 않으므로 원본 상태 그대로 출력
		for(int c : arr) {
			System.out.print(c + " ");
		}
		System.out.println();
