...
class FriendInfoHandler {
	...
	/*
	 ...
	 부모로 자식을 차조할 수 있으므로 부모타입의 배열에는 자식타입의 인스턴스를 모두 저장할 수 있다
	 */
	private Friend[] myFriends;
	/*
	 배열에 저장된 연락처 정보를 카운트하기 위한 변수로, 추가할때마다 ++(증감연사자)로 1씩 증가시킨다.
	 */
...
//		System.out.println("addFriend 호출됨");
		//고딩 또는 대딩 모두 기본정보가 있으므로 먼저 입력받는다
		Scanner scan = new Scanner(System.in);
		...
		if(choice==1) {
			//고딩인 경우 별명을 추가로 입력 받은 후 
			System.out.print("별명 : "); iNickname=scan.nextLine();
			//High 인스턴스를 생성하고 ..
			HighFriend high = new HighFriend(iName, iPhone, iAddr, iNickname);
			//Friend 타입의 인스턴스 배열에 추가
			myFriends[numOfFriends++] = high;
			/*
			 인덱스로 사용된 변수의 초기값이 0이므로, 0번 인덱스에 추가된 후 1증가한다
			 이를 위해 '후위증가'하고 있다
			 */
		} else if(choice==2) {
			//대딩인 경우 전공을 추가로 입력받고 ..
	...
	/*
	 저장된 친구의 정보를 출력하기 위한 멤버 메소드
	 1. 친구정보를 추가할때 High 혹은  Univ 인스턴스를 배열에 저장
	 2. 이때 인스턴스는 Friend로 자동형변환(업캐스티이)되어 저장
	 3. 정보 출력시 배열에 입력된 개체수만큼 반복하여 각 인자를 통해 정보를 출력한다
	 4. 출력을 위한 멤버 메소드는 상속관계에서 오버라이딩 되어있으므로
	  	참조변수의 영향없이 항상 자식클랙스에 오버라이딩 된 메소드가 호출
	 5. 저장된 인스턴스는 Friend 타입이지만 오버라이딩을 통해 별도의 형변환이 필요하지 않다
	  	항상 원하는 정보를 간단히 출력
	 */
	public void showAllData() {
	...
	public void showSimpleData() {
		...
		/*
		 만약 부모클래스인 Friend에 showBasicInfo()를 정의하지 않았다면, 자식클래스에서는 오버라이딩을 할 수 없으므로
		 개별적인 메소드로 정의해야 한다
		 정보가 저장되는 배열은 Friend 타입이므로 자식인스턴스를 저장하면 자동으로 형변환(업캐스팅) 되어 저장된다
		 자식의 정보를 출력하는 메소드를 바로 호출할 수 없으므로  아래와 같이 일일히 확인한 후 강제형변환(다운캐스팅) 해야한다
		 상속의 구조가 복잡해질수록 더 많은 조건식을 추가해야한다
		...
		*/
		...
		/*
		 오버라이딩을 이용하면 상속의 구조가 복잡해지더라도 아래와 같이 출력문장 한줄이면 간단히 처리할 수 있다
		 */
		for(int i=0; i<numOfFriends; i++) {
	...
	public void searchInfo() {
		...
		for(int i=0; i<numOfFriends; i++) {
			/*
			 ...
			 검색을 위해 입력한 이름과 비교해서 일치하는 경우에는 정보를 출력
			 compareTo 대신 equals를 사용해도 결과는 동일
			 */
	...
	public void deleteInfo() {
		...
		/*
		 배열의 원소 중 삭제된 것의 인덱스값을 저장하기 위한 변수로 삭제한 후 빈자리를 채워놓기 위해 사용
		 인데스는 0부터 시작이므로 -1을 초기값으로 지정
		 */
		int deleteIndex = -1;
		...
		for(int i=0; i<numOfFriends; i++) {
			...
			if(deleteName.compareTo(myFriends[i].name)==0) {
				/*
				 인스턴스 배열에서 삭제는 null값을 변경하면 된다
				 참조값이 null이라는 것은 참조할 인스턴스가 없다는 의미이므로 삭제한 것과 동일한 결과가 된다
				 실제로 참조되지 않는 인스턴스는 "가바지 컬렉션"에 의해 자동으로 삭제
				 */
				myFriends[i]=null;
				//삭제된 원소의 index를 저장
				deleteIndex = i;
		...
		if(deleteIndex==-1) {
			...
			/*
			 인스턴스 배열에서 원소를 삭제한 후 바로 뒷부분에 있는 원소를 하나씩 앞으로 복사
			 만약 이 부분이 처리되지 않으면 차후 검색이나 출력시 NullPointerException이 발생할 수 있다
			 */
