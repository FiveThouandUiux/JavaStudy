...
/*
 시나리오]
  ...
  - 행동의 조건
  	...
  	무한히 먹을 수 없고, 에너지는 마이너스가 될 수 없다.
 */
public class Human {
	
	/*
	 멤버변수 : 객체의 속성을 표현
	 	나이의 겨우 산술연산이 필요치 않다면 String으로 설정하는 것을 추천
	 	에너지는 증감 연산이 필요하므로 반드시 int로 설정
	 */
	String name;
	int age;
	int energy;
	...
	void Walking() {
		/*
		 '걷기'를 표현한 메소드로, 호출 될 때마다 에너지를 1 차감
		 만약 차감한 결과가 음수라면 해당 시나리오의 조건을 위배하므로 0으로 고정
		 */
		energy--;
		if(energy<0) {
	...
	void thinking() {
		...
		System.out.println("[walk]에너지가 2 감소하였습니다");
		}
	}
	
	//'먹는 것'을 표현, 2 증가 후 10을 초과한다면 고정
	void eating() {
		energy += 2;
		if(energy>10) {
			energy=10;
