...
/*
 캡슐화(Encapsulation)
  : 여러가지 업무 로직을 하나로 묶는다는 의미와 업무의 순서까지 고려한 형태의 로직을 구성한다는 의미
  관련있는 로직을 하나로 묶어주는 것을 말한다.
 */

//아래 4가지 업무는 번호순서대로 진행해야 한다고 가정
class MemberRegist {
	void doMemberRegist() {
...
/*
 어부의 순서를 고려하여 관련된 메소드를 하나의 클래스로 정의
 차후에는 별도의 분선 절차없이 해당 클래스의 doProcess() 메소드만 호출하면 업무는 차질없이 진행
 */
class EncapsulLogic {	
	//각 업무를 담당할 클래스를 통해 인스턴스를 생성
...
public class E05Encapsulation {
	...
	public static void main(String[] args) {
		...
		congratulationPoint.doCongratulationPoint();//축하포인트 지급
		...
		//아래 메소드 호출을 통해 업무를 순서에 맞게 진행 할 수 있다
