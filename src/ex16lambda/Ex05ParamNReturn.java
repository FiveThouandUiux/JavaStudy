...
public class Ex05ParamNReturn {
	...
	public static void main(String[] args) {
		...
		String s2 = "람다식입니다";
		/*
		 2개의 매개변수는 String으로 추상메소드를 통해 유추할 수 있으므로 람다식에서는 생략 가능
		 메소드명을 제거한 후 괄호 사이에 화살표를 삽입하여 람다식을 정의
		 호출시에는 인터페이스에 정의된 추상메소드를 이용하면 된다
		 */
		StringConcat sc = (a,b) -> {
		...
		/*
		 매개변수가 없는 람다식을 정의해야 하므로 빈 소괄호를 사용
		 난수를 생성해서 반환하는 기능을 정의
		 */
		System.out.println("\n### 람다식2");
		...
		/*
		 람다식 호출시 두 정수를 인수로 전달한 후 이차원 배열을 반환 받는다
		 */
		String[][] returnStr = gugudanRange.guguString(4, 6);
		//반환된 결과값을 통해 24단에서 6단까지 출력
		for(int i=4; i<=6; i++) {
			for(int j=1; j<=9; j++) {
				System.out.println(returnStr[i-1][i-1]+" ");
			}
			System.out.println();
		}
		
		//단순히 문자열만 출력하는 람다식을 정의
		System.out.println("\n### 람다식4");
		MenuPrint menuPrint = () -> {
