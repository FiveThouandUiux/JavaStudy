...
/*
 HashMap<K,V> : Map<T> 인터페이스를 구현한 컬렉션
  ...
  - key값은 중복 될 수 없다, 만약 중복되면 기존의 데이터를 덮어쓰기한다
...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 객체 저장 : 저장시 기존에 저장된 동일한 key값이 존재하면 저장된 인스턴스가 반환된다
		  		만약 처음 입력이라면 null으로 반환한다
		 */
		//처음 저장이므로 null이 반환된다
		System.out.println("name이라는 키값으로 저장된 이전의 값 : "+ map.put("name", "홍길동"));
		/*
		 int 즉 Integer는 String으로 형변환을 할 수 없으므로 타입에러가 발생
		 */
		int number=20;
//		map.put("age", number);
		...
		/*
		 출력하기
		 	1. 키값을 알고 있을때 : get(key값)으로 출력한다
		 */
		System.out.println("키값을 알때 : "+map.get("name")); //최길동
	
		/*
			2. 키값을 모를때(혹은 너무 많아서 하나씩 명시하기 힘들때) 
			 : keySet() 메소드를 통해 전체 key를 Set 컬렉션으로 얻어온다
			 해당 반환값을 확장 for문으로 반복하고 이를 통해 Value를 인출 할 수 있다
		 */
		Set<String> keys = map.keySet();
		System.out.println("[확장for문 적용]");
		for(String key : keys) {
			//get(key값)을 통해 value를 출력
			String value = map.get(key);
			System.out.println(String.format("%s:%s",key,value));
		}
		
		/*
			3. 이터레이터를 통한 출력 : keySet()을 통해 key를 먼저 얻어온 후 반복 출력한다
		 */
		...
		/*
		 Value만 얻어와야 할 때 : vlaues() 메소드를 통해 값을 인출할 수 있따
		 확장 for문을 통해 출력한다
		 */
		...
		/*
		 존재유무 확인하기 
		  : Map컬렉션은 Key와 Value가 있으므로 메소드도 2개로 나눠져있다
		 */
		...
		System.out.println(map.containsValue("남자") ? "남자 있다" : "님자 없다");
		System.out.println(map.containsValue("여자") ? "여자 있다" : "여자 없다");
		
		/*
		 인스턴스 삭제 : key를 통해 삭제하고 삭제에 성공하면 해당 Value가 반환된다
		 */
		System.out.println("삭제된 객체 : "+map.remove("age")); //20출력
