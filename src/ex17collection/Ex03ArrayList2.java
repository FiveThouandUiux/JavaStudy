...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		 아래의 2개의 컬렉션은 내부적인 자료구조만 다를 뿐 구현방법에는 차이가 없다
		 인스턴스 생성시 Student형의 타입매개변수를 통해 생성한다
		 Student 인스턴스를 저장할 수 있는 상자가 된다
		 */
		LinkedList<Student> list2 = new LinkedList<Student>();
//		ArrayList<Student> list2= new ArrayList<Student>();
		
		//List에 저장할 Student 인스턴스 생성 외부 패키지에 선언되었으므로 import 후 생성해야 한다
		...
		//List에 인스턴스 추가. 인덱스는 자동으로 부여
		list2.add(st1);
		list2.add(st2);
		list2.add(st3);
		list2.add(st4);
		//List는 중복저장이 가능한다. 인데스로 구분할 수 있다
		list2.add(st2);
		
		System.out.println("[출력1-일반for문]");
		/*
		 일반 for문 사용을 위해 size() 메소드로 추가된 인스턴스의 갯수를 반환 반는다
		 또한 index로 접근해야 하므로 get() 메소드를 사용한다
		 */
		for(int i=0;i<list2.size();i++) {
			/*
			 List의 각 인덱스에 저장된 인스턴스의 참조값을 그대로 출력
			 toString() 메소드를 오버라이딩 했으므로 인스턴스에 저장된 멤버변수값이 출력
			 만약 오버라이딩이 되어있지 않다면 참조값이 출력
			 */
			System.out.println(list2.get(i));
		}
		
		/*
		 인스턴스의 참조값을 통해 삭제하는 경우 boolean 값이 반환
		 또한 동일한 인스턴스가 존재한다면 앞에 있는 것이 먼저 삭제
		 동시에 삭제되지 않는다
		 */
		...
		/*
		 시나리오] 컬렉션에 저장된 인스턴스를 이름으로 검색하여 삭제나는 코드를 작성하시오
		...
		삭제를 위해 입력한 이름과 Student 인스턴스에 저장된 name과 일치하는 지 확인한 후 해당 인스턴스를 찾을 수 있다
		*/
