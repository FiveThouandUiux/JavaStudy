...
/*
 HashSet<E> : set계열의 컬렉션
  ...
  - 기본클래스(Stringm Integer)인 경우 별도의 처리없이 중복이 제거된다
  - 개발자가 정의한 클래스라면 hashCode(), equals()메소드를 적절히 오버라이딩 해야만 중복을 제거할 수 있다
...
//		HashSet<Object> set = new HashSet<>();
		...
		while(itr.hasNext()) {
			...
			/*
			 HashSet<E>에 저장시 모두 Object현으로 자동형변환되어 저장되므로 우리가 직접 정의한 클래스에 대해서만
			 instaceof를 통해 타입을 확인한 후 다운캐스팅하여 출력한다
			 만약 다운캐스팅을 하지 않으면 자식멤버에 접근할 수 없으므로 getName()을 호출할 수 없다
			 */
