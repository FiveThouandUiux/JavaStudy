...
/*
 TreeSet<T> 컬렉션
  ...
  - set의 기본적인 특성은 동일하다
  - 중복은 허용되지 않는다
  - 인스턴스가 정렬되어 저장된다, 정렬의 기준은 개발자가 직접 정의할 수 있다
 */

/*
 클래스 생성시 정렬의 기준을 만들어주기 위해서는 Comparable<T> 인터페이스를 구현해야한다
 */
class MyString implements Comparable<MyString> {
	...
	//멤버 메소드 : 문자열의 길이를 반환한다
	public int getLength() {
		return str.length();
	}
	
	/*
	 TreeSet<T>에 객체를 저장할 때 정렬을 위해 compareTo()메소드를 오버라이딩한다
	 정령의 기준은 단순하게 가나다와 같은 순서가 될 수도 있고 문자영ㄹ의 길이에 따른 정렬이 될수도 있다
	 아래의 코드는 오름차순 정렬일때 사용한다
	 1과 -1의 위치가 바뀌면 내림찬순이 된다
	 */
	@Override
...
public class Ex05TreeSetMain {
	...
	public static void main(String[] args) {
		...
		/*
		 Java에서 제공하는 기본클래스인 경우 별도의 오버라이딩 없이도 정령되어 저장된다
		 */
